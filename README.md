...
- 🔄 **双向通信** - 轻松接收和回复来自用户的消息
- 🚫 **一键停止转发** - 点击转发消息下方的按钮，让机器人静默丢弃该账号的后续消息
...
9. 要让停止转发名单和用户名索引可靠生效，请创建一个 Cloudflare KV namespace，并在 **Settings** > **Bindings** 中以 `BLOCKLIST` 为变量名绑定它。没有此绑定时，这些状态只在当前运行实例中保留，Worker 冷启动或请求落到其他实例后会失效。
...
点击消息下方的 **🚫 停止转发** 按钮后，机器人仍会收到该账号发来的消息，但会在 Worker 中静默丢弃，不再转发给您。这不是 Telegram 账号封禁；Telegram Bot API 不支持机器人封禁私聊用户。

停止转发后，原消息下方的按钮会变为 **✅ 恢复转发**，点击即可移出名单并重新接收该账号的消息。管理员也可以在与 Bot 的私聊中发送：
...
升级后必须重新访问一次安装地址，让 Telegram webhook 开始接收 `callback_query`。如果省略这一步，按钮会一直转圈且没有任何效果。

Cloudflare 部署必须配置 `BLOCKLIST` KV 才能可靠跨请求保存名单。未配置时，按钮会明确提示“仅当前运行实例有效”；冷启动或请求落到其他实例后，消息可能再次被转发。
